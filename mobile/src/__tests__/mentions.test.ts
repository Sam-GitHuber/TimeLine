/**
 * Typing an `@mention` (Phase 9b M8) — the string half.
 *
 * These are unit tests for the same reason `messageText`'s are: every
 * interesting case is about *characters and a caret* (an email address that
 * isn't a mention, a name deleted after being picked), and staging each one
 * through a rendered composer would bury the thing under test. The screen-level
 * flow — type, pick, send, highlight — is pinned in `thread.test.tsx`.
 *
