/**
 * Which tick a message gets (Phase 9b M4).
 *
 * A unit test rather than a screen test, because the interesting cases are all
 * about *who counts* — a member added after the message, one who opted out, one
 * between spells of membership — and staging those through a rendered thread
 * would bury the rule under fixtures.
 */
...
function mine(created_at = SENT): Message {
  return {
    id: 100,
...
function member(overrides: Partial<Participant> & { id: number }): Participant {
  return {
...
it('never waits on you', () => {
  ...
  // message you sent from a second device stuck on one tick.
  const people = [
...
it('never waits on someone who joined after the message', () => {
  // The audience half. Someone added yesterday was not shown last week's
  // message, so requiring them to have "read" it means a tick that can never
  // complete — and crediting them with it would be a lie.
  const people = [
    member({ id: 2, last_read_at: LATER }),
...
it('never waits on a member who has opted out', () => {
  ...
  const optedOut = member({ id: 3 });
  delete optedOut.last_read_at;
  delete optedOut.active_since;
  const people = [member({ id: 2, last_read_at: LATER }), optedOut];
  expect(readStateFor(mine(), people, ME)).toBe('read');
});
...
describe('receiptsVisible', () => {
  it('is false when you have turned receipts off', () => {
    ...
    // ever opening these".
    const off = [member({ id: ME }), member({ id: 2 })].map((p) => {
      const copy = { ...p };
      delete copy.last_read_at;
      delete copy.active_since;
      return copy;
    });
    expect(receiptsVisible(off)).toBe(false);
  });
