 ...
 * which models the service, the access group and the accessibility. Against the
 * old flat-Map double every one of these would have passed no matter what the
 * code did.
 */
...
/** The one stored item, whatever it was stored under. */
function stored() {
  const entries = (
    SecureStore as unknown as {
      __entries: () => { key: string; service: string; accessible: string }[];
    }
  ).__entries();
  return entries.find((entry) => entry.key === PREVIEW_CREDENTIAL_KEY) ?? null;
}
...
describe('savePreviewCredential', () => {
  ...
  it("pins the keychain service rather than taking expo's default", async () => {
    ...
    // has to be one this repo chose.
    await savePreviewCredential(CREDENTIAL);

    expect(stored()?.service).toBe(PREVIEW_KEYCHAIN_SERVICE);
  ...
  it('replaces the previous credential rather than accumulating copies', async () => {
    ...
    const entries = (
      SecureStore as unknown as { __entries: () => { key: string }[] }
    ).__entries();
    expect(entries.filter((e) => e.key === PREVIEW_CREDENTIAL_KEY)).toHaveLength(1);
    expect(await getPreviewCredential()).toBe(CREDENTIAL);
  });
});
...
describe('the account session', () => {
  it('is not stored alongside it', async () => {
    ...
    // token storage onto these options.
    ...
    const account = entries.filter((e) => e.key.startsWith('timeline.access'));
    expect(account).toHaveLength(1);
    expect(account[0].service).not.toBe(PREVIEW_KEYCHAIN_SERVICE);
    expect(account[0].accessible).toBe(SecureStore.WHEN_UNLOCKED);
  });
});
