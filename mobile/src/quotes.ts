/**
 * Resolving the message a reply quotes (Phase 9b M5).
 *
 ...
 * vetted.
 *
 * Until M5 that somewhere was "messages this screen has already loaded", which
 * was complete only because the transcript eagerly loaded **every** page of
 * history. M5 makes paging lazy, and that quietly breaks the honesty of the
 * fallback: a miss now also means "hasn't paged in yet", so "Original message
 * unavailable" — which is supposed to mean *you were clipped out of this* —
 * would be a lie some of the time, and a message that lies sometimes is worth
 * nothing in the case where it's true.
 *
 ...
 * honest again.
 *
 * **A module store, the same shape as `outbox.ts` and `drafts.ts`**, rather than
...
const resolved = new Map<number, Map<number, Message>>();
 ...
 */
const NONE: ReadonlyMap<number, Message> = new Map();

function resolvedFor(conversationId: number): ReadonlyMap<number, Message> {
  return resolved.get(conversationId) ?? NONE;
}

function useResolved(conversationId: number): ReadonlyMap<number, Message> {
...
export function clearQuotes() {
  ...
  resolved.clear();
  asked.clear();
  conversations.forEach((id) => listeners.get(id)?.forEach((n) => n()));
}

/** Look up a quoted message by id, or `undefined` if it can't be resolved. */
export type QuoteResolver = (id: number) => Message | undefined;
...
export function useQuotedMessages(
  ...
  return (id: number) => loadedById.get(id) ?? cache.get(id);
}
