...
class StaleAuthCookieTests(APITestCase):
    ...
    def _token_for_deleted_user(self):
    ...
    def test_login_succeeds_despite_a_cookie_for_a_deleted_user(self):
        ...
        self.client.cookies[AUTH_COOKIE] = self._token_for_deleted_user()
    ...
    def test_protected_endpoint_treats_a_deleted_user_as_anonymous(self):
        # Access is unchanged — still refused. What changed is *why*: the
        # request is now rejected as "nobody is logged in" rather than dying at
        # the auth layer. Asserting the code (not just the 401) is what makes
        # this a real regression guard: the stock class also 401s here, so a
        # status-only assertion would pass with the fix reverted.
        self.client.cookies[AUTH_COOKIE] = self._token_for_deleted_user()
    ...
    def test_a_valid_cookie_still_authenticates(self):
        # Positive control. Every other test here asserts a *refusal*, so
        # without this one an authenticate() that returned None unconditionally
        # would leave the whole class green while breaking every login.
        self.client.cookies[AUTH_COOKIE] = str(AccessToken.for_user(self.user))
    ...
    def test_bearer_token_for_a_deleted_user_still_401s(self):
        ...
        token = self._token_for_deleted_user()
    ...
    def test_login_recovers_under_real_csrf_enforcement(self):
        # The other tests use the default APIClient, which sets
        # _dont_enforce_csrf_checks — i.e. they all run in a mode production
        # never uses. That matters here specifically: JWT_AUTH_COOKIE_USE_CSRF
        # is on, so the parent runs enforce_csrf() BEFORE the user lookup, and
        # it raises PermissionDenied, which ResilientJWTCookieAuthentication
        # deliberately does not catch. If that ordering ever changed, the
        # recovery path could break in production with the suite still green.
        client = APIClient(enforce_csrf_checks=True)
        client.cookies[AUTH_COOKIE] = self._token_for_deleted_user()

        # Priming CSRF must work too. /api/auth/csrf/ is AllowAny, but the stale
        # cookie used to 401 even *this* — so the SPA couldn't obtain a CSRF
        # token on load, let alone reach login. Asserting it here keeps the
        # failure legible instead of a bare KeyError on the cookie below.
        primer = client.get(CSRF_URL)
    ...
    def test_csrf_is_still_enforced_despite_the_stale_cookie(self):
        # The flip side: relaxing the *authentication* failure must not relax
        # CSRF. A mutating request with no CSRF token is still refused (403
        # from enforce_csrf, which fires before the user lookup) rather than
        # sailing through as anonymous.
        client = APIClient(enforce_csrf_checks=True)
        client.cookies[AUTH_COOKIE] = self._token_for_deleted_user()
